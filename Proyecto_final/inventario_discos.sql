...
--5. Insertar discos 
INSERT INTO disco_musica (nombre_disco, precio, artista, fecha_lanzamiento, peso_MB, genero, stock) VALUES
    ('Thriller', 15.99, 'Michael Jackson', '1982-11-30', 120, 'Pop', 120),
...
INSERT INTO disco_musica (nombre_disco, precio, artista, fecha_lanzamiento, peso_MB, genero, stock) VALUES
    ('azul', 20, 'Bad bunny', '2025-12-25', 100, 'Trap', 10)
select * from auth
select * from usuario
INSERT INTO usuario (cedula, nombre_usuario, edad, genero, correo, id_perfil1)
VALUES (1234567890, 'Billy Garzon', 30, 'Masculino', 'billy.garzon@test.com', 3)
RETURNING id_usuario;

CREATE EXTENSION pgcrypto;
INSERT INTO usuario (nombre_usuario, correo, id_perfil1)
VALUES ('Billy Garzon', 'billygarzon@test.com', 3)
RETURNING id_usuario;
INSERT INTO auth (id_usuario, password_hash)
VALUES (
    18, 
    encode(digest('billy123', 'sha256'), 'hex')
);

DELETE FROM usuario
WHERE id_usuario = (SELECT id_usuario FROM usuario WHERE nombre_usuario = 'Billy Garzon');
SELECT id_usuario FROM usuario WHERE nombre_usuario = 'Billy Garzon';
DELETE FROM auth
WHERE id_usuario = 18;
DELETE FROM usuario
WHERE id_usuario = 16;
...
DELETE FROM usuario
WHERE id_usuario = (SELECT id_usuario FROM usuario WHERE nombre_usuario = 'Paul Pareja');
SELECT id_usuario FROM usuario WHERE nombre_usuario = 'Paul Pareja';
DELETE FROM auth
WHERE id_usuario = 19;
DELETE FROM usuario
WHERE id_usuario = 19;
...
INSERT INTO usuario (nombre_usuario, correo, id_perfil1)
VALUES ('Paul Pareja', 'paulpareja@test.com', 3)
RETURNING id_usuario;
INSERT INTO auth (id_usuario, password_hash)
VALUES (
    20, 
    encode(digest('paul123', 'sha256'), 'hex')
);

UPDATE usuario
SET nombre_usuario = 'Paul Pareja',
    correo = 'paulpareja@test.com',
    id_perfil1 = 2
WHERE nombre_usuario = 'Paul Pareja' 
RETURNING id_usuario